...
export namespace ClaudeProject
{
	...
	export interface Interface
	{
		...
		readonly mcp: Effect.Effect<
			Option.Option<Mcp.McpJsonFile>,
			McpConfigError
		>;
		readonly plugin: Effect.Effect<
			Plugin.LoadedPlugin,
			import('./Errors.ts').PluginLoadError
		>;
		readonly skill: (
			name: string,
		) => Effect.Effect<
			Option.Option<Plugin.PluginSkillEntry>,
			import('./Errors.ts').PluginLoadError
		>;
		readonly command: (
			name: string,
		) => Effect.Effect<
			Option.Option<Plugin.PluginCommandEntry>,
			import('./Errors.ts').PluginLoadError
		>;
		readonly agent: (
			name: string,
		) => Effect.Effect<
			Option.Option<Plugin.PluginAgentEntry>,
			import('./Errors.ts').PluginLoadError
		>;
		readonly outputStyle: (
			name: string,
		) => Effect.Effect<
			Option.Option<Plugin.PluginOutputStyleEntry>,
			import('./Errors.ts').PluginLoadError
		>;
	...
	export const layer = (
		...
		Layer.effect(
			...
			Effect.gen(function*()
			{
				...
				const [settings, invalidateSettings] = yield* Effect
					.cachedInvalidateWithTTL(
						providePlatform(Settings.load(options.cwd)),
						Duration.infinity,
					);
				const [mcp, invalidateMcp] = yield* Effect
					.cachedInvalidateWithTTL(
						providePlatform(
							Effect.gen(function*()
							{
				...
				const [plugin, invalidatePlugin] = yield* Effect
					.cachedInvalidateWithTTL(
				...
				const skill = (name: string) =>
					plugin.pipe(
						Effect.map((loaded) =>
							Arr.findFirst(loaded.skills, (entry) =>
								entry.name === name)
						),
					);
				const command = (name: string) =>
					plugin.pipe(
						Effect.map((loaded) =>
							Arr.findFirst(loaded.commands, (entry) =>
								entry.name === name)
						),
					);
				const agent = (name: string) =>
					plugin.pipe(
						Effect.map((loaded) =>
							Arr.findFirst(loaded.agents, (entry) =>
								entry.name === name)
						),
					);
				const outputStyle = (name: string) =>
					plugin.pipe(
						Effect.map((loaded) =>
							Arr.findFirst(loaded.outputStyles, (entry) =>
								entry.name === name)
						),
					);
 ...
 * @since 0.1.0
 */
export const project: Effect.Effect<
	ClaudeProject.Interface,
	never,
	ClaudeProject.Service
